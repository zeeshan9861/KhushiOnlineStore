...
class CartManager extends BaseManager {
  ...
  generateOrderMessage(customerData) {
    ...
    if (!name || !whatsapp || !address) {
      ...
      if (!whatsapp) missingFields.push('WhatsApp Number');
      if (!address) missingFields.push('Delivery Address');
      this.log(`⚠️ Missing fields: ${missingFields.join(', ')}`);
      return null;
    }

    // Validate WhatsApp number format (should be at least 10 digits)
    const phoneOnly = whatsapp.replace(/\D/g, '');
    if (phoneOnly.length < 10) {
      this.log('⚠️ Invalid WhatsApp number. Please enter a valid phone number.');
      return null;
    }

    // Simple aligned columns (plain text) format
    ...
    const colWidths = { sr: 5, name: 20, qty: 8, price: 12 };

    // Create header matching requested example
    let orderTable = 'SR No. Product name         Qty     Price       \n';

    // Add each item row (aligned)
    this.items.forEach((item, idx) => {
      const srNo = String(idx + 1);
      const productName = item.name.length > colWidths.name ? item.name.substring(0, colWidths.name - 2) + '..' : item.name;
      const qty = String(item.quantity);
      const priceStr = '₹' + item.getSubtotal().toLocaleString('en-IN');

      const row = padRight(srNo, colWidths.sr) + padRight(productName, colWidths.name) + padRight(qty, colWidths.qty) + padLeft(priceStr, colWidths.price);
      orderTable += row + '\n';
    });
  ...
  redirectToWhatsApp(message) {
    ...
    try {
      const phone = this.config.BUSINESS_PHONE.replace(/[^\d]/g, '');
      
      console.log('🚀 WhatsApp Redirect');
      console.log('Phone:', phone);
      
      // Create the URL with encoded message text
      ...
      // Method 1: Try creating and clicking a link element (most reliable)
      try {
        ...
        console.warn('Link method failed:', linkErr);
...
class AppController {
  ...
  handleWhatsAppOrder() {
    ...
    // Validate fields before creating customer data object
    if (!name) {
      this.renderer.showError('❌ Please enter your full name');
      return;
    }

    if (!whatsapp) {
      this.renderer.showError('❌ Please enter your WhatsApp number (include country code, e.g. +91)');
      return;
    }

    if (!address) {
      this.renderer.showError('❌ Please enter your complete delivery address with pin code');
      return;
    }

    // Additional validation for WhatsApp number format
    const phoneOnly = whatsapp.replace(/\D/g, '');
    if (phoneOnly.length < 10) {
      this.renderer.showError('❌ Invalid WhatsApp number. Please enter a valid phone number with at least 10 digits');
      return;
    }

    if (this.cartManager.isEmpty()) {
      this.renderer.showError('❌ Your cart is empty. Please add products before placing an order');
      return;
    }
    ...
    if (!message) {
      this.renderer.showError('❌ Unable to generate order message. Please check all fields and try again');
      return;
    }
    ...
    // Show info message
    const businessPhone = this.cartManager.config.BUSINESS_PHONE;
    const cleanPhone = businessPhone.replace(/[^\d]/g, '');
    const encodedManual = encodeURIComponent(message);
    const manualLink = `https://wa.me/${cleanPhone}?text=${encodedManual}`;
    
    // Minimal, non-intrusive notification: no verbose HTML
    try {
      if (this.renderer && typeof this.renderer.showInfo === 'function') {
        this.renderer.showInfo('Opening WhatsApp...');
      } else if (this.renderer && typeof this.renderer.showMessage === 'function') {
        this.renderer.showMessage('Opening WhatsApp...');
      } else {
        // Fallback: tiny console log only (no modal)
        console.log('Opening WhatsApp... (message copied to clipboard)');
      }
    } catch (notifyErr) {
      console.log('Opening WhatsApp... (notification suppressed)');
    }

    // Attempt to open WhatsApp
    setTimeout(() => {
      console.log('Attempting WhatsApp redirect...');
      const result = this.cartManager.redirectToWhatsApp(message);
